...
/* Horizontal tables embedded in vertical layout */
:is(figure, div, section, article, center).table-horizontal {
  writing-mode: horizontal-tb;
  display: block;
  margin-block: 1rem;
  margin-inline: auto;
  ...
  inline-size: min(calc(var(--vs-page--height) - 6rem), 19rem);
  max-inline-size: min(calc(var(--vs-page--height) - 6rem), 19rem);
  max-block-size: calc(var(--vs-page--width) - 1.5rem);
  border: var(--vs-border-width) solid rgba(0, 0, 0, 0.35);
...
:is(figure, div, section, article, center).table-horizontal>table,
table.table-horizontal {
  width: 100%;
...
:is(figure, div, section, article, center).table-horizontal>table caption,
table.table-horizontal caption {
  caption-side: top;
  margin-bottom: 0.6em;
  text-align: center;
  font-size: 0.85rem;
  font-weight: var(--vs-font-weight-bold, 600);
}

:is(figure, div, section, article, center).table-horizontal>table th,
:is(figure, div, section, article, center).table-horizontal>table td,
table.table-horizontal th,
table.table-horizontal td {
  border: var(--vs-border-width) solid rgba(0, 0, 0, 0.45);
...
:is(figure, div, section, article, center).table-horizontal>table th:first-child,
:is(figure, div, section, article, center).table-horizontal>table td:first-child,
table.table-horizontal th:first-child,
table.table-horizontal td:first-child {
  text-align: left;
}

:is(figure, div, section, article, center).table-horizontal>table td[colspan],
:is(figure, div, section, article, center).table-horizontal>table th[colspan],
table.table-horizontal td[colspan],
table.table-horizontal th[colspan] {
  text-align: left;
}

:is(figure, div, section, article, center).table-horizontal>table td:last-child sup,
table.table-horizontal td:last-child sup {
  font-size: 0.7em;
}

.table-horizontal.table-hyoka {
  max-inline-size: calc(var(--vs-page--height) - 1rem);
}

.table-horizontal.table-hyoka table {
  font-size: 0.75rem;
}

.table-horizontal.table-hyoka .table-section>th {
  background-color: rgba(0, 0, 0, 0.08);
  text-align: left;
  font-weight: var(--vs-font-weight-bold, 600);
  padding-block: 0.45em;
  border-bottom: var(--vs-border-width) solid rgba(0, 0, 0, 0.45);
}

.table-horizontal.table-hyoka .table-subtotal>td:first-child {
  text-align: left;
}

.table-horizontal.table-hyoka .table-summary>td {
  text-align: left;
  font-weight: var(--vs-font-weight-bold, 600);
}
...
.table-image {
  ...
  /* 上下に0.5remずつの余白を確保（画像がページの端に接触するのを防ぐ） */
  padding: 0.5rem 0;

  /* 問題の原因となっていた display: flex を削除 */
}
...
.table-image img {
  ...
  /*
   *【最重要】高さを100%にすることで、親要素（figure）の
   * paddingを除いた領域いっぱいに広げる
   */
  height: 821px;

  /* 幅はアスペクト比を維持するように自動計算 */
  width: 631px;

  /*
   * ブロック要素を水平方向に中央揃えするための、古くからある確実な方法
   */
  margin: 0 auto;
}
...
:is(#toc, [role='doc-toc']) a[href$='#part5'] {
  font-size: 1.2em;
  /* font-weight: var(--vs-font-weight-bold); */
}

:is(#toc, [role='doc-toc']) a[href$='#part5']::after {
  content: none;
}
